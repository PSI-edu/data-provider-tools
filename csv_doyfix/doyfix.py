...
def main():
    parser = argparse.ArgumentParser(
        description="Convert the specified columns in a csv file from doy to yyyy-mm-dd"
    )
    ...
    parser.add_argument(
        "--output-format",
        type=str,
        help="The format of the datetime to convert to",
        default="%Y-%m-%dT%H:%M:%S",
    )
    parser.add_argument(
        "--input-format",
        type=str,
        help="The format of the datetime to convert from",
        default="%Y-%jT%H:%M:%S",
    )
    parser.add_argument(
        "--header",
        action="store_true",
        help="The header row is the first row of the csv file",
    )
    parser.add_argument(
        "--columns",
        type=int,
        nargs="+",
        help="The column numbers to convert",
        required=True,
    )
...
def validate(args):
    ...
    if any(column < 0 or column >= len(row) for column in args.columns):
        out_of_range_columns = [
            column for column in args.columns if column < 0 or column >= len(row)
        ]
        print(
            f"Columns must be within the range of the csv file. Columns {out_of_range_columns} are out of range"
        )
        sys.exit(1)
...
def do_conversion(
    input_file, output_file, columns, input_format, output_format, header
):
    ...
    with open(input_file, "r") as input_file, open(output_file, "w") as output_file:
        ...
        for row in reader:
            for column in columns:
                row[column] = datetime.datetime.strptime(
                    row[column], input_format
